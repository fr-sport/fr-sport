...
styleSheet.innerText = `
    ...
    .pitch-player-img { width: 42px; height: 42px; border-radius: 50%; border: 2px solid #fff; background: #eee; object-fit: cover; box-shadow: 0 3px 8px rgba(0,0,0,0.5); }
    ...
    try {
        ...
        if(m.events && m.events.length > 0) {
            m.events.forEach(ev => {
                ...
                if (ev.type === 'Goal') { icon = svgIcons.goal; if(ev.assist.name) eventBody += `<span class="assist-text" style="font-size:9px; color:gray;">بمساعدة: ${ev.assist.name}</span>`; } 
